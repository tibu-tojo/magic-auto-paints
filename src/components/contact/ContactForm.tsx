...
export function ContactForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      form.reset();
      setStatus({ type: "success", message: "Thank you! We will get back to you as soon as possible." });
    } catch (error: unknown) {
  ...
  return (
    <form className="surface grid gap-5 p-6 sm:p-8" onSubmit={handleSubmit} noValidate>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="form-label">E-mail *<input className="form-control" name="email" type="email" autoComplete="email" maxLength={160} required /></label>
      </div>
      ...
      <label className="form-label">Message *<textarea className="form-control min-h-40 resize-y" name="message" minLength={10} maxLength={2000} required /></label>
      <label className="flex items-start gap-3 text-sm leading-6 text-white/55"><input className="mt-1 h-4 w-4 accent-orange-500" name="consent" type="checkbox" required/><span>I hereby agree that this data will be stored and processed for the purpose of establishing contact. I am aware that I can revoke my consent at any time. *</span></label>
      <p className="text-xs text-white/40">* Indicates required fields</p>
      <button className="button button-primary sm:justify-self-start" disabled={status.type === "sending"} type="submit">{status.type === "sending" ? <LoaderCircle className="animate-spin" size={17}/> : <ArrowRight size={17}/>} {status.type === "sending" ? "Sending" : "Send"}</button>
